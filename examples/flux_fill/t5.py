...
def load_t5_encoder(name: str, max_len: int = 256) -> ml.models.Model:
...
def load_t5_tokenizer(backend: ml.Backend, max_seq_len: int, name: str, pad: bool = True):
    model_file = hf_hub_download(configs[name].repo_id, "tokenizer_2/spiece.model")
    return T5Tokenizer(
        model_file, backend, max_seq_len
    )
#  61588